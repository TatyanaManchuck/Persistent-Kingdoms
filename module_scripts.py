      ...
      (try_begin), # store extra information for script game_get_item_extra_text, called by the show_item_details operation
        ...
        (try_begin),
          ...
          (try_begin), # if a crafting stockpile, calculate the extra reward
            ...
            (call_script, "script_scene_prop_get_item_crafting_refund_reward", ":instance_id", 0),
  ...
  ("game_receive_network_message", # called by the game whenever a custom network message is received, both clients and servers
   [(store_script_param, ":sender_player_id", 1),
    ...
    (try_begin), # section of events received by clients from the server
      ...
      (else_try),
        (eq, ":event_type", server_event_return_chest_gold),
        (store_script_param, ":chest_gold", 3),
        (assign, "$g_chest_gold", ":chest_gold"),
      (try_end),
    ...
    (else_try), # section of events received by server from the clients
      ...
      (try_begin), # handle players requesting to attach a cart to themselves or a horse
        ...
        (try_end),
      (else_try),
        (eq, ":event_type", client_event_receive_chest_gold),
        (store_script_param, ":instance_id", 3),

        (call_script, "script_scene_prop_get_owning_faction", ":instance_id"),
        (assign, ":castle_slot", reg0),
        ...
        (troop_get_slot, ":instance_id2", "trp_mission_data", ":castle_slot"),
        (gt, ":instance_id", 0),
        (scene_prop_get_slot, ":chest_gold", ":instance_id2", slot_scene_prop_stock_count),

        (multiplayer_send_int_to_player, ":sender_player_id", server_event_return_chest_gold, ":chest_gold"),
  ...
  ("cf_craft_item_stockpile", # server: handle players crafting items at a stockpile
   [(store_script_param, ":agent_id", 1), # must be valid
    ...
    (try_begin),
      ...
      (call_script, "script_scene_prop_get_item_crafting_refund_reward", ":instance_id", ":player_id"),
      (call_script, "script_player_adjust_gold", ":player_id", reg0, 1),
      (val_add, ":stock_count", 1),
      (scene_prop_set_slot, ":instance_id", slot_scene_prop_stock_count, ":stock_count"),
      (multiplayer_send_3_int_to_player, ":player_id", server_event_scene_prop_set_slot, ":instance_id", slot_scene_prop_stock_count, ":stock_count"),
   ...
   [(store_script_param, ":instance_id", 1), # must be valid
    ...
    (try_begin),
      ...
      (try_begin),
        ...
        (is_between, ":castle_slot", slot_mission_data_castle_owner_faction_begin, slot_mission_data_castle_owner_faction_end),
        (val_add, ":castle_slot", slot_mission_data_castle_money_chest_begin),
        (troop_get_slot, ":chest_instance_id", "trp_mission_data", ":castle_slot"),
        (gt, ":instance_id", 0),
        (scene_prop_get_slot, ":chest_gold", ":chest_instance_id", slot_scene_prop_stock_count),
      (try_end),
      (try_begin),
       (eq, ":player_id", 0),
       (multiplayer_send_int_to_server, client_event_receive_chest_gold, ":instance_id"),
       (assign, ":chest_gold", "$g_chest_gold"),
      (try_end),
      (try_begin),
        (gt, ":gold_reward", ":gold_value"),
        (store_sub, ":compare_reward", ":gold_reward", ":gold_value"),
        (try_begin),
          (gt, ":chest_gold", ":compare_reward"),
          (val_sub, ":chest_gold", ":compare_reward"),
          (try_begin),
            (neq, ":player_id", 0),
            (scene_prop_set_slot, ":chest_instance_id", slot_scene_prop_stock_count, ":chest_gold"),
          (try_end),
        (else_try),
          (assign, ":gold_reward", ":gold_value"),
        (try_end),
      (try_end),
   (else_try),
